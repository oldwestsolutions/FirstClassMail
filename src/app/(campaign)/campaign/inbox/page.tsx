    ...
    <CampaignShell title="Inbox">
      <div className="grid min-h-[70vh] grid-cols-1 gap-4 lg:grid-cols-12">
          ...
          <ul className="space-y-1 text-sm">
            {folders.map((f) => (
              <li key={f.id}>
                <button
                  type="button"
                  className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-left text-neutral-300 hover:bg-white/5"
                >
                  {f.label}
                  {f.n > 0 && <span className="text-xs text-teal-400">{f.n}</span>}
                </button>
              </li>
            ))}
          </ul>
              ...
              <li key={msg.id}>
                <button
                  ...
                  className={`w-full rounded-l-lg border-l-2 py-3 pl-3 text-left text-sm ${
                    sel === msg.id ? 'border-teal-400 bg-white/[0.06] text-white' : 'border-transparent text-neutral-400 hover:bg-white/[0.03]'
                  }`}
                >
        ...
        <GlassCard className="lg:col-span-6 flex flex-col">
          <div className="mb-4 flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="flex items-center gap-2 font-serif text-xl text-white">
                {m.from}
                <span className="rounded bg-teal-500/20 px-2 py-0.5 text-[10px] font-sans text-teal-300">ZK</span>
              </h2>
              <p className="mt-1 text-sm text-neutral-400">{m.sub}</p>
            </div>
          ...
          <div className="flex-1 rounded-xl border border-white/5 bg-black/30 p-4 text-sm leading-relaxed text-neutral-300">
            {m.prev} Full message body would render here with CMS-safe formatting and audit hash footer.
          </div>
          <div className="mt-4 flex gap-2 border-t border-white/10 pt-4">
            <input className="flex-1 rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-white" placeholder="Reply…" />
            <button type="button" className="rounded-xl bg-teal-600 px-4 py-2 text-sm text-white">
              Send
            </button>
          </div>
        </GlassCard>
